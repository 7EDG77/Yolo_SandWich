...
if __name__ == "__main__":
    # 设置包含图片的文件夹路径
    img_folder = "test1"  # 替换为实际的文件夹路径
    onnx_model_name = "D:\\PyCharm Community Edition 2024.1.4\\PythonPreject\\Awesome-Backbones-main\\Yolov8\\runs\\detect\\train87\\weights\\dynamic_quantized.onnx"
    ...
    # 获取文件夹中所有图片的路径
    img_extensions = ('.png', '.jpg', '.jpeg')  # 支持的图片格式
    img_paths = [os.path.join(img_folder, f) for f in os.listdir(img_folder) if f.lower().endswith(img_extensions)]

    # 遍历所有图片路径
    for img_path in img_paths:
        # 创建YOLOv8实例
        detection = YOLOv8(args.model, img_path, args.conf_thres, args.iou_thres)
        # 模型推理
        output_image = detection.main()

        # 保存结果图片到同一文件夹，文件名前添加'output_'
        output_img_path = os.path.join(img_folder, 'output_' + os.path.basename(img_path))
